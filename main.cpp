...
public:
    ...
    bool isValidMove(char player, int eil, int stulp) {
        ...
        char opponent = (player == 'B') ? 'W' : 'B'; // ? - T ar F
        const int directions[8][2] = {
            {0, 1}, {0, -1}, {1, 0}, {-1, 0},    // hor ir vert
            {1, 1}, {1, -1}, {-1, 1}, {-1, -1}   // istrizai
        };

        // visų direkcijų patikrinimas
        for (int d = 0; d < 8; d++) {
            int e = eil + directions[d][0];
            int s = stulp + directions[d][1];
            bool foundOpponent = false;
...
public:
    ...
    void Winner() {
        ...
        cout << "Žaidėjas B (juodas): " << blackScore << " taškai\n";
        cout << "Žaidėjas W (baltas): " << whiteScore << " taškai\n";
        
        if (blackScore > whiteScore) {
            cout << "Žaidėjas B (juodas) laimėjo!" << endl;
        }
        else if (whiteScore > blackScore) {
            cout << "Žaidėjas W(baltas) laimėjo!" << endl;
        }
        else {
            cout << "Lygiosios!" << endl;
        }
